            ...
            <div className="space-y-4">
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs space-y-4">
                <div className="flex items-center gap-3 border-b border-slate-200 pb-3">
                  ...
                  <div>
                    <h3 className="font-bold text-sm text-slate-900">External Industrial Catalog Importer</h3>
                    <p className="text-slate-500 text-xs">Extract and ingest technical catalog from external endpoints or CSV files.</p>
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="border border-slate-200 rounded-lg p-4 bg-slate-50 space-y-3">
                    <h4 className="font-bold text-xs text-slate-900">Target Scraper: engr.kholifa.com/eeepl/</h4>
                    <p className="text-slate-600 text-xs leading-relaxed">
                      ETL pipeline that connects, paginates through product series, extracts manufacturer part numbers (MPNs), downloads technical datasheets, and validates attributes.
                    </p>
                    <button
                      onClick={() => {
                        setImportStatus('Fetching categories and 4,800+ models from engr.kholifa.com/eeepl/... Schema mapped successfully.');
                      }}
                      className="px-4 py-2 bg-[#0F172A] hover:bg-slate-800 text-white rounded text-xs font-bold flex items-center gap-2 cursor-pointer"
                    >
                      <RefreshCw className="w-3.5 h-3.5 text-amber-400" />
                      <span>Run Remote Extraction Pipeline</span>
                    </button>
                  </div>

                  <div className="border border-slate-200 rounded-lg p-4 bg-slate-50 space-y-3">
                    <h4 className="font-bold text-xs text-slate-900">Bulk CSV / Excel Upload</h4>
                    <p className="text-slate-600 text-xs leading-relaxed">
                      Upload standard CSV containing columns: MPN, Manufacturer, Category, Series, Rated Current, Breaking Capacity, Voltage, Image URL.
                    </p>
                    <div className="flex items-center gap-2">
                      <input type="file" className="text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded file:border-0 file:text-xs file:font-bold file:bg-slate-200 file:text-slate-700" />
                      <button 
                        onClick={() => setImportStatus('Imported 125 validated models into industrial catalog.')}
                        className="px-3 py-1.5 bg-[#D97706] hover:bg-[#B45309] text-white text-xs font-bold rounded cursor-pointer"
                      >
                        Ingest
                      </button>
                    </div>
